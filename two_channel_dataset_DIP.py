...
Kspace_data_name = 'data/NEW_KSPACE'
...
kspace_data_from_file = np.load(os.path.join(Kspace_data_name, kspace_file), allow_pickle=True).item()
kspace_data.append(kspace_data_from_file)

mask_vali =[]
mask_data_name = 'data/MRI_sampling/4_accerlation_mask'

if not os.path.exists(mask_data_name):
    os.makedirs(mask_data_name)
    print(f"创建文件夹: {mask_data_name}")
else:
    print(f"文件夹已存在: {mask_data_name}")
...
mask_from_file = np.load(os.path.join(mask_data_name, mask_file))
